 ...
 * MessageFormat - Selects the format in which messages are sent to the broker,
 *          human or json. Defaults to human.
 * HostTags -When set, HostTags is added to the end of the metric.
 *   It is intended to be used for adding additional metadata to the metric
 * with.
 *   Dots and whitespace are not escaped in this string.
 *   When MessageFormat is set to "human" text should be in valid syslog
 *   structured data format. No spaces allowed.
 *   Example:
 *     [ovirt engine="test"][ovirt vm="test2"]
 *   When MessageFormat is set to "json", text should be in valid Json format.
...
#ifndef WS_DEFAULT_FORMAT
#define WS_DEFAULT_FORMAT "human"
#endif

#ifndef WS_DEFAULT_PREFIX
...
static int ws_send_message(const char *key, const char *value, cdtime_t time,
  ...
  if (strcasecmp("JSON", msg_format) == 0) {
    ...
    status = snprintf(
         ...
         * MessageFormat = "human" */
        message, sizeof(message),
        "<166>1 %s %s collectd - - [%s value=\"%s\""
        " plugin=\"%s\" plugin_instance=\"%s\""
        " type_instance=\"%s\" type=\"%s\""
        " ds_name=\"%s\" interval=\"%.0f\"] %s %s.%s=\"%s\"\n",
        buffer, host, metrics_prefix, value, plugin, plugin_instance,
        type_instance, type, ds_name, CDTIME_T_TO_DOUBLE(interval), host_tags,
        plugin, key, value);
  }
  if (status < 0)
    return -1;
